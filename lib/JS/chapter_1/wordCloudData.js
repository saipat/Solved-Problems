...
class WordCloudData {
    ...
    addWordToMap(word) {
        ...
        if (this.wordsToCounts.has(word)) {
            newCount = this.wordsToCounts[word]+ 1;
...
// test
let inputString = "After beating the eggs, Dana read the next step: Add milk and eggs, then add flour and sugar."


const p = new WordCloudData(inputString);
console.log(p.populateWordsToCounts(inputString));

inputString = 'Hi hi bi'
const p1 = new WordCloudData(inputString);
console.log(p1.populateWordsToCounts(inputString));
