...
class AnimalShelter{
    ...
    dequqeCat(){
        if(!this.catList.front){
            throw new Error('There is no cat to get');
        }
    ...
    dequqeAny(){
        ...
        // console.log(this.catList.peek().date);

        if(firstCat && firstDog){
            // console.log(`firstCat => ${firstCat.value}`);
            
            if (firstCat.date.getTime() < firstDog.date.getTime()){
